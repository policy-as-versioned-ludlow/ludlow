#!/usr/bin/env bash
# Idempotent, offline-safe bring-up of the ludlow institution:
...
say "seeding ludlow gitops repo + building offline git server"
rm -rf "$WORK"; mkdir -p "$WORK/seed" "$WORK/ctx"
cp -R "$GITOPS_DIR/." "$WORK/seed/"
git -C "$WORK/seed" init -q -b main
git -C "$WORK/seed" -c user.email=demo@ludlow -c user.name=demo add -A
git -C "$WORK/seed" -c user.email=demo@ludlow -c user.name=demo commit -q -m "ludlow gitops @1.0.0"
...
SEED_TAG=v1.0.0
git -C "$WORK/seed" -c user.email=demo@ludlow -c user.name=demo tag -a "$SEED_TAG" -m "ludlow $SEED_TAG"
COMMIT="$(git -C "$WORK/seed" rev-parse HEAD)"
say "pinned revision: $SEED_TAG @ ${COMMIT}"
git -C "$WORK/seed" clone -q --bare "$WORK/seed" "$WORK/ctx/ludlow.git"
cp "$GITSERVER_DIR/Dockerfile" "$GITSERVER_DIR/lighttpd.conf" "$WORK/ctx/"

# 3b. seed ludlow's pinned nist dependency the same way, from estate/nist ---
...
kind: GitRepository
metadata: { name: ludlow, namespace: flux-system }
spec:
  interval: 1m
  url: ${GIT_URL_IN_CLUSTER}
  ref:
...
spec:
  ...
  retryInterval: 1m
  timeout: 3m
  sourceRef: { kind: GitRepository, name: ludlow }
  path: ./apps
  prune: true
  wait: true
YAML
...
flux reconcile source git ludlow --context "$CTX"
flux reconcile source git nist --context "$CTX"
flux reconcile kustomization ludlow --with-source --context "$CTX"
