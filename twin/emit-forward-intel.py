#!/usr/bin/env python3
"""Render ludlow's forward-intel feed from ludlow's own twin overlay -- or refuse, and say why.
...
Run it (needs the hub's `twin` package and pyyaml):
    ...
    .venv/bin/python .estate-clone/ludlow/twin/emit-forward-intel.py           # write
    .venv/bin/python .estate-clone/ludlow/twin/emit-forward-intel.py --check   # compare only
"""
...
CLAIM_EXCLUDED = ["uk-gdpr", "pci-dss"]
CLAIM_NOTE = (
    "The shock is an exfiltration of the member health record store, scoped to the one regime this "
    "plan is supervised under. UK data protection and the card schemes are carved out: this party "
    "administers a US health plan and no publisher in its inherits[] prices either, so a number "
    "here would have no instrument behind it."
)
...
RESPONSE_ID = "run-the-claims-adjudication-at-%s"
