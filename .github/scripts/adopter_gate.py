...
answer to the same question has no tie-breaker"). Instead it:
  ...
  4. For every changed version, finds platform's committed evidence file and
     ...
     HOW that one committed file reaches cosign: until 2026-09-06 this
     gate got it wrong for every bundle platform has ever published (it
     passed --trusted-root with --new-bundle-format=true, and cosign
     v3.1.3 -- the version shift-left.yml installs by checksum -- answers
     "--trusted-root only supported with --new-bundle-format" to the
     LEGACY bundle shape platform publishes); from 2026-09-06 it fed the
     legacy path's own env vars, one key per role selected by log id; since
     eco-system ticket 105 (2026-09-09) every bundle takes the
     --trusted-root door, a legacy one re-encoded locally first, so the
     whole root -- every log, every validFor window -- is what verifies.
     See the ticket-105 block above verify_evidence() for the door and
     what the env-var pin cost; the accept and the refusals are proved
     against platform's real published bundles, offline, in
     verify-adopter-gate.sh Part E.
...
def verify_evidence(platform_dir: Path, commit: str, version: str,
    ...
    # Ticket 105: every bundle takes the one door that reads the whole committed
    # root (the block above). A legacy bundle is re-encoded first; a shape this
    # gate cannot read is refused by name, before cosign is called.
    try:
        verify_bundle, flags, env = pinned_verification(bundle_text, evidence_path.read_bytes(), workdir)
    except ValueError as exc:
        raise Refused(
            f"policy {version}: the committed bundle at computed-semver/evidence/{version}.json"
            f".bundle (commit {commit}) {exc}") from exc

    result = _run([
        "cosign", "verify-blob",
        f"--bundle={verify_bundle}",
        *flags,
...
def render_comment(old_pin: dict, new_pin: dict, declared: str, composed: str,
                    results: list[dict], note: str | None, acceptance: dict | None = None) -> str:
    ...
    if acceptance is not None:
        ...
        # acceptance records, read at the head the gate graded. The table above keeps "major".
        ...
        lines += [f"**Composed major, {verdict}** (records under `accepted-majors/`):"]
        lines += [f"- {line}" for line in acceptance.get("lines") or []]
        lines += [""]
...
def run(ludlow_dir: Path, platform_dir: Path, old_ref: str, new_ref: str,
    ...
    try:
        ...
        acceptance = None
        if composed == "major":
            # Eco-system ticket 132: read the acceptance records at the head this gate grades.
            # ludlow's own ref in both paths: --composed-head-ref, or --new-ref without it.
            head = composed_head_ref if composed_head_ref is not None else new_ref
            majors = [r["version"] for r in results if r["kind"] == "changed" and r["bump"] == "major"]
            try:
                records = acceptance_records_at(ludlow_dir, head)
            except ValueError as exc:
                raise Refused(f"could not read the acceptance records: {exc}") from exc
            acceptance = acceptance_verdict(records, PARTY, majors, retired)
            acceptance["lines"] = acceptance_lines(acceptance, PARTY, head)
        comment = render_comment(old_pin, new_pin, declared, composed, results, note, acceptance)

        if composed == "major" and acceptance is not None:
            if acceptance["admitted"]:
                print(f"PASS: declared={declared} composed=major, admitted: every major this pull "
                      f"request adds is accepted by a record in {RECORD_DIR}/ (eco-system ticket 132) "
                      f"-- " + "; ".join(acceptance["lines"]))
                return 0, comment
            print(f"REFUSE: composed bump is major -- {[r['version'] for r in results if r['bump']=='major']} "
                  f"-- not every major it brings is accepted -- " + "; ".join(acceptance["lines"]))
            return 1, comment
        if note:
            print(f"INFO: {note}")
...
def selfcheck() -> None:
    ...
    assert bundle_shape('["a", "list"]') is None

    # Ticket 105: the re-encoding is graded on bytes this test lays down
    # itself -- every field of the served legacy bundle lands where the v0.1
    # Sigstore bundle carries it, the only computed field is the artefact's
    # own digest, and a bundle missing any field refuses by name.
    ...
    pem = "-----BEGIN CERTIFICATE-----\n" + base64.b64encode(b"DER").decode() + "\n-----END CERTIFICATE-----\n"
    converted_pem = json.loads(sigstore_bundle(
        planted.replace(base64.b64encode(b"DER").decode(), base64.b64encode(pem.encode()).decode(), 1), b"artefact"))
    assert converted_pem["verificationMaterial"]["x509CertificateChain"]["certificates"][0]["rawBytes"] == base64.b64encode(b"DER").decode()
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            assert "neither a legacy" in str(exc) and "refusing rather than handing cosign no trust root" in str(exc), exc

    print("OK: the bundle's own shape chooses whether it is re-encoded, a legacy bundle re-encodes field "
          "for field into the v0.1 Sigstore bundle the pinned --trusted-root door reads, a bundle missing "
          "any of those fields refuses by name, and a shape this gate cannot read is refused before cosign")
    ...
    for records, why in (([], "no record"),
                         ([(here, _rec(party="tuppence"))], "another party"),
                         ([(here, _rec(version="5.0.1"))], "another version"),
                         ([(here, _rec(publisher="nist"))], "another publisher"),
                         ([(here, _rec(by="''"))], "a malformed record")):
        v = acceptance_verdict(records, "ludlow", ["5.0.0"], [])
        assert v["admitted"] is False and [u["version"] for u in v["unaccepted"]] == ["5.0.0"], (why, v)
    assert any("tuppence" in n for n in acceptance_verdict(
        [(here, _rec(party="tuppence"))], "ludlow", ["5.0.0"], [])["unaccepted"][0]["near"])
    v = acceptance_verdict([(here, _rec())], "ludlow", ["5.0.0"], ["4.0.0"])
    assert v["admitted"] is False and v["retired"] == ["4.0.0"], v
    v = acceptance_verdict([(here, _rec())], "ludlow", ["4.0.0", "5.0.0"], [])
    assert v["admitted"] is False and [u["version"] for u in v["unaccepted"]] == ["4.0.0"], v
    assert acceptance_verdict([], "ludlow", [], ["4.0.0"])["admitted"] is False
    ...
    print("OK: ticket 132 -- a major is admitted only by a record for ludlow, platform and that exact "
...
# and every major version it adds is accepted for ludlow, for platform, at
...
# copied byte for byte; the hub check fails ludlow if the copy differs.
# --------------------------------------------------------------------------
