#!/usr/bin/env bash
# Beat: "ludlow's twin loads its own overlay, says whose money and in which currency, prices no
...
for f in sorted(os.listdir(comp_dir)):
    ...
    comps[doc["id"]] = doc
FLOOR = {"claims-adjudication": "the caged workload", "cage-policy-line": "the pinned policy line",
         "member-record-store": "the regulated data",
         "plan-administration-fees": "the declared cash flow"}
...
unpriced = [t for t in rungs if ("run-the-claims-adjudication-at-%s" % t) not in priced]
...
try:
    ...
    planted("a ladder rung the overlay prices no response for",
            lambda p: os.remove(os.path.join(p, "twin", "orgs", ORG, "responses",
                                             ("run-the-claims-adjudication-at-%s" % "quarantine") + ".yaml")),
...
cat "$log"
case $rc in
  0) echo "PASS: ludlow's twin overlay loads, is labelled, prices every rung it declares and emits its feed";;
