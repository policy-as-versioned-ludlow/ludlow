...
want "$tol" "$want_tol" "risk-appetite configmap drifted from ludlow's own signed party.yaml appetite"
echo "   reconcile assertions 1-6 all observed true: ludlow reconciles from a pinned GitRepository ($SELF_TAG), healthy, content live, nist dependency pinned ($NIST_TAG), risk skin (Deny-heavy (strictest)) live."
...
# Kyverno at all. ludlow's Namespace carries policy-as-versioned.dev/governed=true
...
if ! timeout 10 kubectl --context "$CTX" get crd mutatingpolicies.policies.kyverno.io >/dev/null 2>&1; then
  skip "Kyverno MutatingPolicy CRD not installed on $CTX, so the composed cage is not enforced there at all: the governed Namespace declares a tier that nothing reads, and no pod in it is caged. An engine on this cluster must be the one gitops/engine/kyverno.yaml declares (hub ADR-0033 point 2), and platform/graded/up.sh then runs against it, as on kind-driftwood"
fi
...
# It does not prove the thing ticket 16 Q1 asks for: that ludlow's COMPOSED POLICY SET is in
