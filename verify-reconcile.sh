#!/usr/bin/env bash
# Beat: "ludlow reconciles from a pinned, signed GitRepository, healthily, and carries its risk skin (Deny-heavy (strictest))."
...
want "$(live flux-system gitrepository ludlow '{.spec.ref.tag}')" "$SELF_TAG" "GitRepository tag"
c=$(live flux-system gitrepository ludlow '{.spec.ref.commit}')
...
kubectl --context "$CTX" --request-timeout=20s get ns ludlow >/dev/null 2>&1 || fail "namespace 'ludlow' not reconciled"
want "$(live ludlow cm ludlow-live-version '{.data.policyVersion}')" "$WANT_VERSION" "live version configmap"
...
want "$(live ludlow cm ludlow-nist-pin '{.data.catalogVersion}')" "$WANT_CATALOG" "nist-pin configmap"
...
tol=$(live ludlow cm ludlow-risk-appetite '{.data.toleranceGBP}')
...
echo "PASS: ludlow reconciles from a pinned GitRepository ($SELF_TAG), healthy, content live, nist dependency pinned ($NIST_TAG), risk skin (Deny-heavy (strictest)) live."
